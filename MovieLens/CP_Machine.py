...
class CP_Machine:
    def __init__(self, rank=5, s_features=[2, 3, 4, 5], init_std=0.001, exp_reg=1.1, reg=0.01, seed=42):
        self.rank =  rank
...
#    
    def build_graph(self):
        with self.graph.as_default():
            ...
            # list of CP Factor Matrices
            self.G = [None]*self.n_features

            #list of CP Factor Matrices used for penalty
            ...
            for i in range(1, self.n_features):
                ...
                self.outputs = tf.multiply(self.outputs, cur_A)
            self.outputs = tf.reduce_sum(self.outputs,axis=1)
            
            # regularization penalty
            self.penalty = tf.matmul(tf.transpose(self.G_exp[0]),self.G_exp[0])
#            
            for i in range(1, self.n_features):
                self.penalty = tf.multiply(self.penalty, tf.matmul(tf.transpose(self.G_exp[i]),self.G_exp[i]))
                
            self.penalty = tf.reduce_sum(self.penalty)
            # MSE loss
